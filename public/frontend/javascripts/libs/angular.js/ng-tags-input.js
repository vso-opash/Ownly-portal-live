...
tagsInput.directive('tagsInput', ["$timeout", "$document", "$window", "$q", "tagsInputConfig", "tiUtil", function($timeout, $document, $window, $q, tagsInputConfig, tiUtil) {
    ...
    return {
        ...
        scope: {
            ...
            tagClass: '&',
            onTagAdding: '&',
            //onTagAdding: '&onTagAdding', //updated by Mohammad
            onTagAdded: '&',
            onInvalidTag: '&',
            onTagRemoving: '&',
            onTagRemoved: '&',
            onTagClicked: '&',   
            scale: '=' //added by Mohammad 
        },
        replace: false,
        transclude: true,
        ...
        controller: ["$scope", "$attrs", "$element", function($scope, $attrs, $element) {
           ...
           // console.log($scope.scale);
            tagsInputConfig.load('tagsInput', $scope, $attrs, {
                ...
                minLength: [Number, 1], // updation  3 >> 1  in order to select C1, C2 from autocomplete
        ...
        link: function(scope, element, attrs, ngModelCtrl) { 
            ...
            //console.log(scope.scale);  
            //update KEYS.up,KEYS.down             
            var hotkeys = [KEYS.enter, KEYS.comma, KEYS.space, KEYS.backspace, KEYS.delete, KEYS.left, KEYS.right,KEYS.up,KEYS.down],
            ...
            scope.$watch('tags.length', function(i) {
                setElementValidity();
                ...
                ngModelCtrl.$validate();
                  //Added by Mohammad
                 if(i!==undefined && i > 0){                   
                   events.trigger('tag-added', { $tag: scope.tags });
                 }
                //Added by Mohammad
            });
            ...
            events
                ...
                .on('input-keydown', function(event) {
                    ...
                    shouldSelect = (key === KEYS.backspace || key === KEYS.left || key === KEYS.right || key===KEYS.up || key===KEYS.down) && scope.newTag.text().length === 0 && !options.enableEditingLastTag;
                    ...
                    if (shouldAdd) {
                        ...
                        if (key === KEYS.left || key === KEYS.backspace || key===KEYS.up) { //update key===KEYS.up
                            tagList.selectPrior();
                        }
                        else if (key === KEYS.right || key===KEYS.down) { //update key===KEYS.down
                            tagList.selectNext();
                        }
                    }
        ...
        link: function(scope, element, attrs, tagsInputCtrl) {
            var tagsInput = tagsInputCtrl.registerTagItem(),
                ...
                if(scope.data.hasOwnProperty("data")){
                    scope.$watch("data.data",function(value){ 
                       //console.log(value);
                     scope.scale = value;    
                    },true);   
                   }
